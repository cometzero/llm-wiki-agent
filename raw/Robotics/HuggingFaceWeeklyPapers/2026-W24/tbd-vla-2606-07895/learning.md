...
- Discrete diffusion / masked token denoising
- VLA policy, action grounding, closed-loop latency
...
이 수식은 “block 내부는 병렬, block 사이 관계는 순차”라는 논문의 핵심을 담는다.
...
3. **TBD-VLA가 자율주행에 주는 힌트는?**  
   waypoint/trajectory token을 block diffusion으로 생성하면 closed-loop latency와 temporal consistency를 함께 다룰 수 있다.
...
- 4차: Discrete Diffusion VLA, Fast-dVLA, ReflectDrive-2와 비교
